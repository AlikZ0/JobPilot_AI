...
| **Свои приоритеты**           | Веса восьми компонентов настраиваются ползунками и пресетами; сохранённые баллы пересчитываются одной кнопкой, без обращения к AI       |
...
| **Отчёт и экспорт**           | Сводка за неделю, какой сайт даёт лучшие совпадения, что чаще всего требуют, выгрузка откликов в CSV и Markdown                         |
...
На обзоре: сводка за неделю, разбор по источникам (какой сайт приносит вакансии,
где вы подходите лучше) и что чаще всего требуют.

Отклики выгружаются в **CSV** для таблицы и в **Markdown** для отчёта. CSV идёт
с BOM и точкой с запятой: без BOM Excel читает кириллицу как мусор, а запятая в
русской локали считается десятичным разделителем и всё съезжает в одну колонку.
...
Веса компонентов — не догма: кому-то важнее вилка, кому-то удалёнка, кому-то
стек. В **Настройки → Что для вас важнее** восемь ползунков и пять готовых
раскладов: «Баланс», «Стек», «Деньги», «Удалёнка», «Рост».

Ползунок задаёт важность, а не баллы: сумма приводится к 100, и рядом написано,
во сколько баллов важность превратилась. Иначе сдвиг одного ползунка молча менял
бы максимумы всех остальных компонентов. Приведение — целочисленное и всегда даёт
ровно 100: сумма весов и есть максимум балла, разъедься она на единицу — и «100 %»
стало бы недостижимым, а разбор перестал бы сходиться с итогом.

Компонент можно увести в ноль. Тогда он не считается вовсе и в разборе так и
написано: «не учитывается». Пропасть без объяснения была бы хуже прочерка.

Смена весов обесценивает кеш анализов — подпись весов входит в его ключ, и разбор,
посчитанный по другим максимумам, переиспользовать нельзя. Но кеш сам по себе
ничего не пересчитывает: список остался бы отсортированным по старым приоритетам,
молча и незаметно. Поэтому рядом есть кнопка **Пересчитать сохранённые баллы** —
она проходит по всем проанализированным вакансиям и переписывает балл. К
AI-провайдеру пересчёт не обращается: качественная часть берётся из сохранённых
выводов модели. Если их не сохраняли (_Приватность → Хранить обоснования AI_),
вакансия считается детерминированно, и в отчёте о пересчёте написано, у скольких
так вышло.
...
Это веса по умолчанию — те, что описаны выше в «Своих приоритетах» и меняются в
настройках.
