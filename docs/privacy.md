...
- **Доступ к сайтам: нет.** Выдаётся по одному origin, когда вы об этом просите.
  В настройках есть справочник карьерных страниц (каталог remoteintech.company) и
  кнопка «Разрешить все сайты» — одно широкое разрешение вместо восьмисот
  отдельных. Это осознанный выбор пользователя: он расширяет право читать
  страницы на весь веб, поэтому рядом с кнопкой стоит предупреждение, а
  умолчание остаётся прежним — не выдано ничего.
...
| Отозвать сразу все доступы       | Настройки → Сайты с вакансиями → Отозвать всё |
